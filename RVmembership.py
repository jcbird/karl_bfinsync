...
for ii, cluster in enumerate(clusters):
    ...
    obs['corr_vrad'].plot(kind='hist', histtype='step', bins=np.arange(-30,50.1,2), label=cluster, ax=np.ravel(axs)[ii])
    np.ravel(axs)[ii].legend()
...
for ii, cluster in enumerate(clusters):
    obs = bycluster.get_group(cluster)
    obs['dvrad'].plot(kind='hist', histtype='step', bins=np.arange(0,10,0.05), label=cluster, ax=np.ravel(axs)[ii])
    np.ravel(axs)[ii].legend()
